...
`make install` will use the loaded custom image, but Compose may choose to build if it is missing. Confirm `docker image inspect local/redmine-dmsf:6.1.4-dmsf4.1.3` succeeds before starting. Do not run `make pull` or rebuild on the offline host.
