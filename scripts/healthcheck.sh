...
check_all() {
  docker compose exec -T postgres sh -c 'PGPASSWORD="$REDMINE_DB_PASSWORD" psql --host=127.0.0.1 --username="$REDMINE_DB_USERNAME" --dbname="$REDMINE_DB_DATABASE" --tuples-only --command="SELECT 1" | grep -q 1' >/dev/null 2>&1 &&
...
failed=0
printf '\n== PostgreSQL authenticated application query ==\n'
if docker compose exec -T postgres sh -c 'PGPASSWORD="$REDMINE_DB_PASSWORD" psql --host=127.0.0.1 --username="$REDMINE_DB_USERNAME" --dbname="$REDMINE_DB_DATABASE" --tuples-only --command="SELECT 1" | grep -q 1'; then echo "OK"; else failed=1; fi
